...
const add = async (req, res) => {
  try {
    ...
    await pool.query(
      `insert into "order" (full_name, email)
    values ($1, $2)`,
      [full_name, email]
    );
...
const get = async (req, res) => {
  try {
    ...
    const argums = await pool.query(`select * from order`);
    if (!argums) {
...
const update = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `update order set full_name = $2 where id = $1`,
      [myId, neww]
    );
    return res
      .status(200)
...
const deleteOne = async (req, res) => {
  try {
    ...
    await pool.query(`delete from order where id = $1`, [myId]);
