...
const add = async (req, res) => {
  try {
    ...
    await pool.query(
      `insert into operation (order_id, description)
    values ($1, $2)`,
...
const get = async (req, res) => {
  try {
    ...
    const argums = await pool.query(`select * from operation`);
    if (!argums) {
...
const update = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `update operation set order_id = $2 where id = $1`,
      [myId, neww]
    );
    return res
      .status(200)
...
const deleteOne = async (req, res) => {
  try {
    ...
    await pool.query(`delete from operation where id = $1`, [myId]);
